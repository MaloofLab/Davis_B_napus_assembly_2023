...
B.napus.convert.bedpe("plotsr/DaAe_Ancestral.bedpe")
...
```{r}
alt.B.napus.convert.bedpe <- function(file){
  df <- read_tsv(file, col_names = F) %>% 
    mutate(X7 = ifelse(X7 == "TRANS" | X7 == "INVTR",
                       paste("TRANS", X1, sep = "-"),
                       X7)
           )
  rdir <- dirname(file)
...
alt.Ancestral.convert.bedpe <- function(file){
  df <- read_tsv(file, col_names = F) %>% 
    mutate(X7 = ifelse(X7 == "TRANS" | X7 == "INVTR",
                       paste("TRANS", X4, sep = "-"),
                       X7)
