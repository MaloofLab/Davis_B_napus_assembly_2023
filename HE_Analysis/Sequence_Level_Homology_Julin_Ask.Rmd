...
for(i in 11:(nrow(Clustered_ZS11_Coverage)-10)){
  Neighbors <- Clustered_ZS11_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_ZS11_Coverage$Consensus[i-1]
  Clustered_ZS11_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_Darmor_Bzh_V10_Coverage)-10)){
  Neighbors <- Clustered_Darmor_Bzh_V10_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Darmor_Bzh_V10_Coverage$Consensus[i-1]
  Clustered_Darmor_Bzh_V10_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_GanganF73_Coverage)-10)){
  Neighbors <- Clustered_GanganF73_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_GanganF73_Coverage$Consensus[i-1]
  Clustered_GanganF73_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_No2127_Coverage)-10)){
  Neighbors <- Clustered_No2127_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_No2127_Coverage$Consensus[i-1]
  Clustered_No2127_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_QuintaA_Coverage)-10)){
  Neighbors <- Clustered_QuintaA_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_QuintaA_Coverage$Consensus[i-1]
  Clustered_QuintaA_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_Shengli3_Coverage)-10)){
  Neighbors <- Clustered_Shengli3_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Shengli3_Coverage$Consensus[i-1]
  Clustered_Shengli3_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_Tapidor_Coverage)-10)){
  Neighbors <- Clustered_Tapidor_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Tapidor_Coverage$Consensus[i-1]
  Clustered_Tapidor_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_Westar_Coverage)-10)){
  Neighbors <- Clustered_Westar_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Westar_Coverage$Consensus[i-1]
  Clustered_Westar_Coverage$Consensus[i] <- mean(Winner)
}

for(i in 11:(nrow(Clustered_Zheyou73_Coverage)-10)){
  Neighbors <- Clustered_Zheyou73_Coverage$Relative_Coverage[(i-10):(i+10)]
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Zheyou73_Coverage$Consensus[i-1]
  Clustered_Zheyou73_Coverage$Consensus[i] <- mean(Winner)
}
