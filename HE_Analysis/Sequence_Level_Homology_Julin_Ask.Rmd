...
Coverage_Readin <- function(filename){
  df <- read_delim(filename,
    ...
    mutate(Genome_Average = median(ifelse(Adjusted_Coverage>4,NA,Adjusted_Coverage), na.rm = T),
           ...
           Relative_Coverage = round(Adjusted_Coverage / Genome_Average, 2),
...
for(i in 11:(nrow(Clustered_Pseudo_napus_Coverage)-10)){
  Neighbors <- round(Clustered_Pseudo_napus_Coverage$Relative_Coverage[(i-10):(i+10)],1)
...
for(i in 11:(nrow(Clustered_DaAe_Coverage)-10)){
  Neighbors <- round(Clustered_DaAe_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
...
GanganF73_Coverage <- Coverage_Readin("Combined_Coverages/Combined_GanganF73_Coverage.tsv.gz")
No2127_Coverage <- Coverage_Readin("Combined_Coverages/Combined_No2127_Coverage.tsv.gz")
QuintaA_Coverage <- Coverage_Readin("Combined_Coverages/Combined_QuintaA_Coverage.tsv.gz")
Shengli3_Coverage <- Coverage_Readin("Combined_Coverages/Combined_Shengli3_Coverage.tsv.gz")
Tapidor_Coverage <- Coverage_Readin("Combined_Coverages/Combined_Tapidor_Coverage.tsv.gz")
Westar_Coverage <- Coverage_Readin("Combined_Coverages/Combined_Westar_Coverage.tsv.gz")
Zheyou73_Coverage <- Coverage_Readin("Combined_Coverages/Combined_Zheyou73_Coverage.tsv.gz")
```
...
GanganF73_Coverage <- GanganF73_Coverage %>%
  mutate(Assembly = "GanganF73",
         Chromosome = str_replace(Chromosome, "ganganF73v0_", "chr"))
No2127_Coverage <- No2127_Coverage %>%
  mutate(Assembly = "No2127",
         Chromosome = str_replace(Chromosome, "no2127v0_", "chr"))
QuintaA_Coverage <- QuintaA_Coverage %>%
  mutate(Assembly = "QuintaA",
         Chromosome = str_replace(Chromosome, "quintaAv0_", "chr"))
Shengli3_Coverage <- Shengli3_Coverage %>%
  mutate(Assembly = "Shengli3",
         Chromosome = str_replace(Chromosome, "shengli3v0_", "chr"))
Tapidor_Coverage <- Tapidor_Coverage %>%
  mutate(Assembly = "Tapidor",
         Chromosome = str_replace(Chromosome, "tapidor3v0_", "chr"))
Westar_Coverage <- Westar_Coverage %>%
  mutate(Assembly = "Westar",
         Chromosome = str_replace(Chromosome, "westarv0_", "chr"))
Zheyou73_Coverage <- Zheyou73_Coverage %>%
  mutate(Assembly = "Zheyou73",
         Chromosome = str_replace(Chromosome, "zheyou73v0_", "chr"))

All_Coverage <- rbind(Pseudo_napus_Coverage,
                      DaAe_Coverage,
                      ZS11_Coverage,
                      Darmor_Bzh_V10_Coverage,
                      GanganF73_Coverage,
                      No2127_Coverage,
                      QuintaA_Coverage,
                      Shengli3_Coverage,
...
Clustered_GanganF73_Coverage <- GanganF73_Coverage %>%
  mutate(Consensus = NA)

Clustered_No2127_Coverage <- No2127_Coverage %>%
  mutate(Consensus = NA)

Clustered_QuintaA_Coverage <- QuintaA_Coverage %>%
  mutate(Consensus = NA)

Clustered_Shengli3_Coverage <- Shengli3_Coverage %>%
  mutate(Consensus = NA)

Clustered_Tapidor_Coverage <- Tapidor_Coverage %>%
  mutate(Consensus = NA)

Clustered_Westar_Coverage <- Westar_Coverage %>%
  mutate(Consensus = NA)

Clustered_Zheyou73_Coverage <- Zheyou73_Coverage %>%
  mutate(Consensus = NA)
...
for(i in 11:(nrow(Clustered_ZS11_Coverage)-10)){
  Neighbors <- round(Clustered_ZS11_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
...
for(i in 11:(nrow(Clustered_Darmor_Bzh_V10_Coverage)-10)){
  Neighbors <- round(Clustered_Darmor_Bzh_V10_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
...
for(i in 11:(nrow(Clustered_GanganF73_Coverage)-10)){
  Neighbors <- round(Clustered_GanganF73_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_GanganF73_Coverage$Consensus[i-1]
  Clustered_GanganF73_Coverage$Consensus[i] <- round(mean(Winner),1)
}

for(i in 11:(nrow(Clustered_No2127_Coverage)-10)){
  Neighbors <- round(Clustered_No2127_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_No2127_Coverage$Consensus[i-1]
  Clustered_No2127_Coverage$Consensus[i] <- round(mean(Winner),1)
}

for(i in 11:(nrow(Clustered_QuintaA_Coverage)-10)){
  Neighbors <- round(Clustered_QuintaA_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_QuintaA_Coverage$Consensus[i-1]
  Clustered_QuintaA_Coverage$Consensus[i] <- round(mean(Winner),1)
}

for(i in 11:(nrow(Clustered_Shengli3_Coverage)-10)){
  Neighbors <- round(Clustered_Shengli3_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Shengli3_Coverage$Consensus[i-1]
  Clustered_Shengli3_Coverage$Consensus[i] <- round(mean(Winner),1)
}

for(i in 11:(nrow(Clustered_Tapidor_Coverage)-10)){
  Neighbors <- round(Clustered_Tapidor_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
...
for(i in 11:(nrow(Clustered_Westar_Coverage)-10)){
  Neighbors <- round(Clustered_Westar_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Westar_Coverage$Consensus[i-1]
  Clustered_Westar_Coverage$Consensus[i] <- round(mean(Winner),1)
}

for(i in 11:(nrow(Clustered_Zheyou73_Coverage)-10)){
  Neighbors <- round(Clustered_Zheyou73_Coverage$Relative_Coverage[(i-10):(i+10)],1)
  #winner <- as.numeric(names(table(neighbors))[table(neighbors)==max(table(neighbors))])
  Winner <- median(Neighbors, na.rm = T)
  if(is_empty(Winner)) Winner <- Clustered_Zheyou73_Coverage$Consensus[i-1]
  Clustered_Zheyou73_Coverage$Consensus[i] <- round(mean(Winner),1)
}

Clustered_All <- rbind(Clustered_Pseudo_napus_Coverage,
                       Clustered_DaAe_Coverage,
                       Clustered_ZS11_Coverage,
                       Clustered_Darmor_Bzh_V10_Coverage,
                       Clustered_GanganF73_Coverage,
                       Clustered_No2127_Coverage,
                       Clustered_QuintaA_Coverage,
                       Clustered_Shengli3_Coverage,
                       Clustered_Tapidor_Coverage,
                       Clustered_Westar_Coverage,
                       Clustered_Zheyou73_Coverage)
Clustered_All$Assembly <- factor(Clustered_All$Assembly, levels = c("Pseudo_napus", "DaAe", "ZS11", "Darmor-Bzh_V10", "GanganF73", "No2127", "QuintaA", "Shengli3", "Tapidor", "Westar", "Zheyou73"))
```
...
```{r}
HEpos <- comparisons %>% 
  filter(assembly %in% levels(Clustered_All$Assembly)) %>%
  select(Assembly=assembly, links) %>% 
  unnest(links) %>%
  mutate(Start=ref.start, 
         End=ref.end,
         Chromosome=str_replace(ref.name, "^.*([AC][0-9]{1,2})", "chr\\1"))
...
for (i in chroms) {
  ...
  ggsave(paste0("Coverage_Plots_20221030/",i,".png"), width = 10, height = 10)
}
