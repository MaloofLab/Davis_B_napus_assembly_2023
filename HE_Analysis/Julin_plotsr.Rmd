...
DaAe.plotsr <- DaAe.plotsr %>%
  ...
  mutate(r.start = r.start - 1,
         ...
         q.start=q.start.new) %>%
    filter(r.chrom==q.chrom | str_sub(r.chrom,1,1) != str_sub(q.chrom, 1,1)) %>% # keep syntenic regions and HE between subgenomes
    mutate(ann=ifelse(str_sub(r.chrom, 1, 3) != str_sub(q.chrom, 1,3),
...
```{r}
load("Julin_Coverages_and_Positions2_median.RData")
Clustered_All <- Clustered_All %>%   mutate(Chromosome=str_remove(Chromosome, "chr"),
  ...
  rowwise() %>%
  mutate(Consensus=min(Consensus, 3.0)) %>% # truncate duplication levels to keep plot scale reasonable
  mutate(Start=floor(mean(c(Start,End))),
         End=Start+1) %>%
  ungroup() 
...
#if we need to bring function from library to repo:
#file.copy("/opt/homebrew/Caskroom/miniconda/base/envs/plotsr/lib/python3.10/site-packages/plotsr/func.py", "plotsr/modified_func.py", overwrite = TRUE)
...
params_cov="--itx --cfg base.cfg -H 6 -W 12 -S 0.5"

# plot the comparisons where we have coverage:
glist="Darmor-Bzh_V10 ZS11 GanganF73 No2127 QuintaA Shengli3 Tapidor Westar Zheyou73"
...
source ~/.bash_profile
conda activate plotsr
params_cov="--itx --cfg base.cfg -H 10 -W 10 -S 0.5 "


plotsr \
  --bp ZS11_Ancestral.bedpe \
  --bp Ancestral_DaAe.bedpe \
  --genomes ZS11_Anc_DaAe_genomes.txt \
  --tracks ZS11_tracks.txt \
  ${params_cov} --chr A01 --chr C01 \
  -o plots/ZS11_Anc_DaAe_A01_C01.pdf

plotsr \
  --bp No2127_Ancestral.bedpe \
  --bp Ancestral_DaAe.bedpe \
  --genomes No2127_Anc_DaAe_genomes.txt \
  --tracks No2127_tracks.txt \
  ${params_cov} --chr A02 --chr C02 \
  -o plots/No2127_Anc_DaAe_A02_C02.pdf
  
plotsr \
  --bp Zheyou73_Ancestral.bedpe \
  --bp Ancestral_DaAe.bedpe \
  --genomes Zheyou73_Anc_DaAe_genomes.txt \
  --tracks Zheyou73_tracks.txt \
  ${params_cov} --chr A02 --chr C02 \
  -o plots/Zheyou73_Anc_DaAe_A02_C02.pdf
...
plotsr \
  ...
  --genomes Zheyou73_Anc_DaAe_genomes.txt \
  --tracks Zheyou73_tracks.txt \
  ${params_cov} --chr A03 --chr C03 \
  -o plots/Zheyou73_Anc_DaAe_A03_C03.pdf
...
plotsr \
  ...
  --genomes ZS11_Anc_DaAe_genomes.txt \
  --tracks ZS11_tracks.txt \
  ${params_cov} --chr A09 --chr C08 \
  -o plots/ZS11_Anc_DaAe_A09_C08.pdf
  
plotsr \
  --bp Darmor-Bzh_V10_Ancestral.bedpe \
  --bp Ancestral_DaAe.bedpe \
  --genomes Darmor-Bzh_V10_Anc_DaAe_genomes.txt \
  --tracks Darmor-Bzh_V10_tracks.txt \
  ${params_cov} --chr A09 --chr C08 \
  -o plots/Darmor-Bzh_V10_Anc_DaAe_A09_C08.pdf
...
plotsr \
  ...
  --genomes ZS11_Anc_DaAe_genomes.txt \
  --tracks ZS11_tracks.txt \
  ${params_cov} --chr A10 --chr C09 \
  -o plots/ZS11_Anc_DaAe_A10_C09.pdf
  
plotsr \
  --bp Tapidor_Ancestral.bedpe \
  --bp Tapidor_DaAe.bedpe \
  --genomes Tapidor_Anc_DaAe_genomes.txt \
  --tracks Tapidor_tracks.txt \
  ${params_cov} --chr A02 --chr C02 \
  -o plots/Tapidor_Anc_DaAe_A02_C02.pdf
  
plotsr \
  --bp QuintaA_Ancestral.bedpe \
  --bp QuintaA_DaAe.bedpe \
  --genomes QuintaA_Anc_DaAe_genomes.txt \
  --tracks QuintaA_tracks.txt \
  ${params_cov} --chr A02 --chr C02 \
  -o plots/QuintaA_Anc_DaAe_A02_C02.pdf
...
plotsr \
  ...
  --genomes QuintaA_Anc_DaAe_genomes.txt \
  --tracks QuintaA_tracks.txt \
  ${params_cov} --chr A05 --chr C05 \
  -o plots/QuintaA_Anc_DaAe_A05_C05.pdf
...
plotsr \
  ...
  --tracks Zheyou73_tracks.txt \
  ${params_cov} --chr A05 --chr C05 \
  -o plots/Zheyou73_Anc_DaAe_A05_C05.pdf
```
