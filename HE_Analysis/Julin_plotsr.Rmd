...
# get order right (flipped)
positions <- match(c("ZS11", "Ancestral", "DaAe"), genome.info$name)

genome.info <- genome.info[positions,]

genome.info

write_tsv(genome.info, "plotsr/genomes_rev.txt")
...
DaAe.plotsr <- DaAe.plotsr %>%
  ...
  mutate(q.chrom=str_remove(q.chrom, "DaAe-"),
...
others.plotsr <- others.plotsr %>%
  ...
  select(r.chrom=to.GR.seqnames,
      ...
      mutate(q.chrom=str_remove(q.chrom, "Anc-"),
             r.chrom=str_replace(r.chrom, "C([1-9])", "C0\\1"),
...
```{r}
Clustered_All %>% filter(Assembly=="DaAe", !is.na(Consensus)) %>%
  select(Chromosome, Start, End, Consensus) %>%
  write_tsv("plotsr/DaAeCov.bedgraph", col_names = FALSE)

Clustered_All %>% filter(Assembly=="ZS11", !is.na(Consensus)) %>%
  select(Chromosome, Start, End, Consensus) %>%
  write_tsv("plotsr/ZS11Cov.bedgraph", col_names = FALSE)
```

```{r}
Clustered_All %>% filter(Assembly=="ZS11", !is.na(Consensus)) %>%
  group_by(Chromosome) %>%
  summarize(max=max(Consensus))
```


## create tracksinfo file

```{r}
tibble(`#file`="DaAeCov.bedgraph", name="DaAE_Coverage", tags="ft:bedgraph;bw:10000") %>%
  write_tsv("plotsr/DaAeTracks.txt")

tibble(`#file`="ZS11Cov.bedgraph", name="ZS11_Coverage", tags="ft:bedgraph;bw:10000") %>%
  write_tsv("plotsr/ZS11Tracks.txt")
```
